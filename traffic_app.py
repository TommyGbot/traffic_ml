...
with st.sidebar.expander("Option 2: 📝 Manual Input (Form)"):
    with st.form("traffic_input_form"):  # <— create a form with a unique key
       ...
       # Get min and max datetime from dataset
        month = st.selectbox("month",default_df['month'].unique())
        weekday = st.selectbox("weekday",default_df['weekday'].unique())
        hour = st.selectbox("Hour", sorted(default_df['hour'].unique()))
...
#    user_df=user_df.drop(columns=['month'])
#    user_df=user_df.drop(columns=['weekday'])
#    user_df=user_df.drop(columns=['hour'])
...
#        combined_df['dayofweek'] = combined_df['date_time'].dt.dayofweek
...
# After creating dummy variables
   # Create dummy-encoded version of combined data
   combined_df_encoded = pd.get_dummies(combined_df)

# Create dummy-encoded version of training features for reference
   train_encoded = pd.get_dummies(novolume)

# Reindex combined_df_encoded to have the same columns as training data
   combined_df_encoded = combined_df_encoded.reindex(columns=train_encoded.columns, fill_value=0)
   original_rows = novolume.shape[0]

# Split back into original and user sets
